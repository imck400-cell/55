...
const LessonPlanner: React.FC<LessonPlannerProps> = ({ onBackToWelcome, lesson }) => {
    ...
    const handleAnalyze = async () => {
        ...
        try {
            ...
            setLessonPlan(prev => ({ ...initialLessonPlan, ...prev, ...partialPlan, id: prev.id, emblem1: prev.emblem1, emblem2: prev.emblem2 }));
        } catch (err) {
            ...
            {!isPrinting && (
                <section className="max-w-7xl mx-auto mb-4 p-4 bg-sky-50 border border-sky-200 rounded-lg">
                    ...
                    <div className="mt-2 flex items-center gap-4">
                        <button onClick={handleAnalyze} disabled={isLoading} className="bg-sky-600 text-white font-bold py-2 px-5 rounded-lg hover:bg-sky-700 disabled:bg-sky-300 transition-all flex items-center">{isLoading ? 'جاري التحليل...' : 'تحليل النص'}</button>
